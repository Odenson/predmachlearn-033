...
* [describe how the model was built](#training-model)
* [what choices were made for this model](#choice-of-prediction-algorithm)
* [how the model was cross validated](#testing)
* [what the expected out of sample error is](#estimation-of-error-rate)
...
outliers but is affected by multi-collinearity, which will [later be
tested](#check-multi-collinearity) for.
...
* testing (30%) - used for [cross-validation](#testing), and estimate of [error rate](#estimation-of-error-rate)
...
Applying these conditions gives the formula:
...
badNames <- sort(names(nas[nas >= 19216]))
goodNames <- setdiff(names(training), badNames)

# exclude columns that do not aid in prediction (or are an outcome)
trainNames <- grep(paste("classe", "window", "user_name", "X", "_timestamp", sep = "|"), goodNames, value = TRUE, invert = TRUE)

# use these column names to generate training formula
trainFormula <- as.formula(paste("classe ~ ", paste(trainNames, collapse = "+"))); trainFormula
```
...
[caret](http://caret.r-forge.r-project.org/) package with:
...
Below is an abbreviated list of the models variable importance:
...
The following scripts and this report are available from GitHub,
[here](https://github.com/frankhjung/predmachlearn-033).
...
* This HTML Report https://frankhjung.github.io/predmachlearn-033/project.html
