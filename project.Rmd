...
conversion error. As a consequence we will exclude from the prediction formula
all columns with a 95% or higher missing data.

The ``classe`` field is a categorical outcome, which we will subsequently treat
as a factor.
...
|--------|--------|
| ``X`` | Row index |
| ``*_window``, ``*_timestamp*`` | measurements were made in time segments with overlap so they can be treated as discrete |
...
A descending of importance list for the first 20 variables is:
...
The final model was run against the validation data with:
...
```{r predictions}
kable(data.frame(id = 1:20, Prediction = answers), caption = "Predictions")
```
