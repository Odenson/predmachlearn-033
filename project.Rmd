...
```{r errorEstimate, echo = TRUE}
# estimate error (since this is categorical data we are estimating accuracy)
missClassified <- function(trueValues, predictValues) {
    sum(trueValues != predictValues) / length(trueValues)
}
errorRate <- missClassified(testing$classe, testPredict)
```

For this model we have an estimated error rate of **`r percent(errorRate)`**.
